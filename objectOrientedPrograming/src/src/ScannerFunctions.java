...
public class ScannerFunctions {
	 ...
	 * 
	 */
	private static void readFileAndAddToList(String fileName, LinkedList<WiFiLinkedList> wifiList, Filter filter) {
		...
		try {
			...
			while (Line != null) {
				...
				if (count > 2) {
					...
					WiFi wf = new WiFi(SSID, MAC, freq, signal, lat, lon, alt, time);

					if (wifiList.size() == 0 && (filter == null || filter.isBelong(wf))) {
					...
					else if (filter == null || filter.isBelong(wf)) {
						if (wll.IsBelong(lat, lon, time)) {
							wll.add(wf); // adding WiFi to WiFiLinkedList
						} else {
							wll = new WiFiLinkedList(lat, lon, alt, time, id);
							wifiList.add(wll); // adding WiFiLinkedList to
							wll.add(wf);
						}
					}
	 ...
	 * @param path
	 *            = path to create the kml file
	 * @param rectTop
	 *            = coordinates for the upper points of the rectangle
	 * @param rectBot
	 *            = coordinates for the lower points of the rectangle
	 */
	public static void printToKmlByGPS(ArrayList<String[]> list, String path, double[] rectTop, double[] rectBot) {
		HashMap<Integer, String> mac = new HashMap<>(); // creating a list of
														// mac
		...
		String temp = "";
		String[] lineMark = { "#", "", "", "", "", "", "-200" }; // used to mark
																	// lines
																	// that
																	// won't get
																	// added to
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = i + 1; j < list.size(); j++) {
				if (temp.equals(mac.get(j)) && i != j) // if the mac address of
														// this line was found
														// in another line
				{
					if (Double.parseDouble(list.get(j)[6]) < Double.parseDouble(list.get(i)[6])) // replace
																									// the
																									// line
																									// with
																									// the
		...
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i) != lineMark) // if the line was not marked add its
											// data to the kml
			{
	 ...
	 * 
	 * @param directoryName
	 *            directory to collect all csv files from
	 * @param csvWritePath
	 *            path to write the merged csv file
	 */
	public static LinkedList<WiFiLinkedList> getAllcsvFilesFromFolderAndAddtoOneCSVTable(String directoryName,String csvWritePath, Filter filter) {
		...
		for (String csvFileName : fileList) {
			readFileAndAddToList(csvFileName, wifiList, filter);
		}
	...
	public static void run() {
		...
		while (!exit) {
			...
			//	Filters.filtercsvFileByGPS(csvWriteFolder, kmlPath, startLon, startLat, endLon, endLat);//
			...
			getAllcsvFilesFromFolderAndAddtoOneCSVTable(folder, csvWriteFolder, filter);
			sort.close();
			csvScanner.close();
			folderScanner.close();
	...
	public static void runTest() {
		String folder = "C:\\Users\\Yarden\\Documents\\testOOP";
		// String folder = "C:\\Users\\USER\\Desktop\\data\\New fcolder";
		String csvWritePath = "C:\\Users\\Yarden\\Documents\\testOOP\\CSVnew.csv";
		/*
		 * String
		 * kmlFolder="C:\\Users\\USER\\Desktop\\New folder\\BookTime.kml";
		 * 2017:11:12:13:06:42 2017:11:12:13:09:42 start
		 * <coordinates>34.97470633453521,32.06320051865906,0</coordinates>
		 * 
		 * end <coordinates>34.87623703518137,32.11926397274331,0</coordinates>
		 */
		...
		case 1: {
			...
			String kmlPath = "C:\\Users\\Yarden\\Documents\\testOOP\\CSVnew.csv";
		...
		case 2: {
			...
			Filters.filtercsvFileByGPS(csvWritePath, kmlPath, startLon, startLat, endLon, endLat);
...
//		Filter filter = new FilterMAC(MAC);
		//Filter filter = new FilterSSID("partner");		
//		Filter filter = new FilterTime("2017-11-12 13:06:05", "2017-11-12 13:06:49");
